	...
	function _recoverTags2html(text) {
		...
		if (_tags4Html) {
			...
			if (parserTable.length > 0) {
				...
				// we put a nbsp there so that the span doesn't get santized away :-)
		...
		function preserveWikiTags4Html(text) {
			...
			text = text.replace(matcher, function(match, $1, $2, $3) {
				...
				if ( $1 == 'ref' ) {
					...
					refHtml = editor.dom.createFragment( refHtml ).firstChild.innerHTML.replace(/([^}]){@@vnl@@}{@@vnl@@}/gmi, '$1<br class="mwt-emptylineFirst"/>');
					refHtml = refHtml.replace(/{@@vnl@@}/gmi, '<br class="mwt-emptyline"/>');
...
//0914									lines[i-1] = lines[i-1] + '<@@slb@@>';
		...
		function newLines2html (text) {
			...
			function paragraphNewLines2html (text) {
				// make a regular expresion matcher to see if a line ends wtth a block element
				// this is used when walking through the editor content line by line.  We add
				// back PRE because it isn't in the block tag list as mediawiki treats them 
				// differently to how browsers do!
				...
				// wrap pseudo blocks in a paragraph 
			...
			function convertNewLines2html( elm, level ) {
				...
				if (( elm[0].tagName == 'TD' ) 
					...
					innerHtml = paragraphNewLines2html( innerHtml );
			...
			// put something in mepty SPANs
			$dom.find( "s" ).replaceWith( function() {
				var elm = this,
					outerHtml = elm.outerHTML,
					innerHtml = elm.innerHTML;
...
debugger;
						if (elm.hasClass( 'mwt-paragraph' )) {
							...
							// not paragraph means it has no new lines before it
...
/*		if ( e.initial ) {
		...
		// this bit of code should remove potential xss vulnerabilities 
		var serializer = new tinymce.html.Serializer(),
			parser = new tinymce.html.DomParser();
			
		e.content = serializer.serialize(parser.parse( e.content ));
...
debugger;
		if ( e.command == "mceInsertContent" ) {
			if (e.value.convert2html) {
				e.value.content = _convertWiki2Html(e.value.content);
			}			
		}
