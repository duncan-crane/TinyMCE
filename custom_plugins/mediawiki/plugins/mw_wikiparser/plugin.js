		...
		function preprocessHtml2Wiki( text ) {
			...
			$dom.find( "*[class*='mwt-preserveHtml']" ).replaceWith( function(a) {
				var newLine,
					...
					tagNewline = '',
					tagSpaces = '',
					id = "<@@@" + this.tagName.toUpperCase() + ":" + createUniqueNumber() + "@@@>",
...
debugger;
	...
	function _onDblClick(evt) {
		...
		targetFound = false;
debugger;
		if ( onDblClickLaunch ( evt.target, "mwt-image", "wikiupload" )) {
			return;
		} else if ( onDblClickLaunch ( evt.target, "mwt-internallink", "wikilink" )) {
			return;
		} else if ( onDblClickLaunch ( evt.target, "mwt-externallink", "wikilink" )) {
			return;
		} else if ( onDblClickLaunch ( evt.target, "mwt-wikiMagic", "wikitextEditor" )) {
			return;
		}
